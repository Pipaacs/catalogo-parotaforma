  ...
  for (let config of configuracionPaginas) {
    ...
    if (contenedor) {
      ...
      if (muebles && muebles.length > 0) {
        muebles.forEach((mueble) => {
          ...
          if (mueble.precio_rebaja && mueble.precio_rebaja < mueble.precio) {
            const descuento = Math.round(
              ((mueble.precio - mueble.precio_rebaja) / mueble.precio) * 100,
            );
        ...
        contenedor.innerHTML =
          '<p style="text-align:center; width:100%;">Aún no hay productos en esta categoría.</p>';
      }
      break;
    }
  }
...
async function cargarDetalleProducto() {
  ...
  document.getElementById("detalle-precio").innerText =
    `$${Number(producto.precio).toLocaleString("es-MX")} MXN`;
  document.getElementById("detalle-descripcion").innerText =
    producto.descripcion || "Sin descripción disponible.";
  document.getElementById("detalle-material").innerText =
    producto.material || "N/A";
  document.getElementById("detalle-acabado").innerText =
    producto.acabado || "N/A";
  document.getElementById("detalle-medidas").innerText =
    producto.medidas || "N/A";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ==========================================
  setTimeout(() => {
    // Lo retrasamos 1 milisegundo para que cargue aislado del código viejo
    try {
      // --- 1. LÓGICA DE LA LUPA ---
      ...
      if (searchInput && searchResults) {
        searchInput.addEventListener("input", async (e) => {
          ...
          searchResults.innerHTML =
            '<p style="text-align:center; width:100%; grid-column: 1/-1;">Buscando...</p>';
          ...
          if (error || !data || data.length === 0) {
            searchResults.innerHTML =
              '<p style="text-align:center; width:100%; grid-column: 1/-1;">No hay resultados.</p>';
            return;
          }
          ...
          data.forEach((m) => {
            ...
            if (m.precio_rebaja && m.precio_rebaja < m.precio) {
              const descuento = Math.round(
                ((m.precio - m.precio_rebaja) / m.precio) * 100,
              );
      ...
      if (window.location.pathname.includes("producto-detalle.html")) {
        ...
        supabaseClient
          ...
          .then(({ data: p, error }) => {
            ...
            document.getElementById("detalle-categoria").innerText =
              p.categoria;
            document.getElementById("detalle-descripcion").innerText =
            ...
            if (p.precio_rebaja && p.precio_rebaja < p.precio) {
              const descuento = Math.round(
                ((p.precio - p.precio_rebaja) / p.precio) * 100,
              );
      ...
      console.error("Error protegido en lupa/detalle:", e);
    }
  }, 1);
  ...
  window.cambiarFoto = function (elemento, srcNueva, index) {
    ...
    document
      .querySelectorAll(".premium-gallery img")
      .forEach((i) => i.classList.remove("thumb-activa"));
  ...
  window.moverGaleria = function (direccion) {
    ...
    if (window.indiceFoto < 0)
      window.indiceFoto = window.galeriaActual.length - 1;
    ...
    if (thumbs[window.indiceFoto]) {
      ...
      thumbs[window.indiceFoto].scrollIntoView({
        behavior: "smooth",
        block: "nearest",
        inline: "center",
      });
